...
src_prepare() {
	...
	# The published binary carries this patch, so modules_prepare has to see the
	# same tree or an external module links against the wrong Module.symvers.
...
src_configure() {
	...
	# Override user variable with the cert used during build. The published
	# package is built with USE=-modules-sign, which leaves the file empty, and
	# then there is no build-time cert to prefer over the user's own.
